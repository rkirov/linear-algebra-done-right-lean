...
/-!
# Skippable infrastructure: the L² inner product space {lit}`C[a,b]`
...
{lit}`C(Set.Icc a b, ℝ)` and provides no measure on the interval subtype, whereas Axler
uses the `L²` inner product `⟨f, g⟩ = ∫ₐᵇ f g`. So we (i) equip the interval
subtype with its Lebesgue measure and (ii) introduce a type synonym {lit}`L2C a b`
...
space instance is gated on {lit}`Fact (a < b)` — for the degenerate interval {lit}`a = b`
the interval subtype is a single point of Lebesgue measure zero, on which
{lit}`⟨f, f⟩ = 0` for every {lit}`f`.
-/
...
/-- The interval subtype embeds measurably into {lit}`ℝ`. -/
...
/-- {lit}`C[a, b]` carrying the `L²` inner product — a type synonym for
{lit}`C(Set.Icc a b, ℝ)` that avoids mathlib's sup-norm instance. -/
...
/-- View an element of {lit}`L2C a b` as an ordinary continuous function. -/
...
/-- The `L²` inner product `⟨f, g⟩ = ∫ₐᵇ f g` on {lit}`C[a, b]`. -/
@[reducible] noncomputable def core [Fact (a < b)] :
    InnerProductSpace.Core ℝ (L2C a b) where
  ...
  smul_left f g c := by
    simp only [toCont_smul, ContinuousMap.smul_apply, smul_eq_mul, starRingEnd_apply,
      star_trivial]
